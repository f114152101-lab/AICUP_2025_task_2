...
if __name__ == "__main__":
    print('訓練集圖片數量 : ',len(os.listdir("./datasets_with_neg_samples_ALL/train/images")))
    print('訓練集標記數量 : ',len(os.listdir("./datasets_with_neg_samples_ALL/train/labels")))
    print('驗證集圖片數量 : ',len(os.listdir("./datasets_with_neg_samples_ALL/val/images")))
    print('驗證集標記數量 : ',len(os.listdir("./datasets_with_neg_samples_ALL/val/labels")))

    print("CUDA 是否可用 (torch.cuda.is_available):", torch.cuda.is_available())
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #model = YOLO('yolov8x.pt')
    model = YOLO(r'D:\Course\AI Friday\task2\runs\detect\train27\weights\best.pt')
    model.to(device)
